...
function TopHu(){
	...
	Promise.all([active1, active2, active3]).then(result => {
		...
		.then(resultArray2 => {
			var temp_data = JSON.stringify({TopHu: {mini_poker: result[0], big_babol: result[1], vq_red: result[2]}});
					...
					TaiXiu_User.findOne({uid: obj.uid}, function(error, data) {
						...
						if(void 0 !== io.users[obj.uid]){
							Promise.all(io.users[obj.uid].map(function(client){
								client.send(JSON.stringify({taixiu:{status:{win:obj.win, select:obj.select, bet: bet}}}));
							}));
						}
							...
							if (TaiXiu_red_lech_tai && TaiXiu_tong_red_lech > 0) {
								if (TaiXiu_tong_red_lech >= obj.bet) {
									...
									if (win) {
										...
										userUpdate['redWin'] = anPhanTram(betwin);
										userUpdate['red'] += userUpdate['redWin'];
